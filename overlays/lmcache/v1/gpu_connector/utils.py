...
# Uses per-engine KV-format discovery required by mixed KDA/MLA cache groups.
...
def _extract_registered_tensor_shapes(
    kv_caches: DiscoverableKVCache,
) -> set[tuple[int, ...]]:
    """Return all leaf tensor shapes in a possibly nested KV-cache value.

    Backport of LMCache 0.5.3's mixed-format detection guard (PR #4206).
    The image's native core is intentionally retained for its DCP patches.
    """
...
def normalize_and_discover_per_layer_formats(
    ...
    # Detect the whole structure only when every leaf has the same shape.
    # Otherwise a detector can mistake a KDA+MLA list for one fused format and
    # reshape tensors using geometry from a different cache group.
    if len(_extract_registered_tensor_shapes(kv_caches)) == 1:
