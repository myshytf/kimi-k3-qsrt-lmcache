...
# Backports vLLM 0.26 unified-Mamba/subpaged-MLA handling from LMCache 0.5.3
# and preserves compatibility with the tested custom native EngineKVFormat.
...
class _MambaUnifiedViewEdit(KVCacheGroupEdit):
    """Re-view vLLM 0.26's unified Mamba state as an opaque one-plane page.
    ...
    Upstream LMCache 0.5.3 converts this to a new 4-D ``*_CS`` engine format.
    The image's DCP-aware native extension predates that enum.  Mamba has one
    synthetic head, so flattening that size-one axis is a zero-copy equivalent:

    - [num_blocks, vllm_block_size, opaque_size_per_token]

    This maps to LMCache's existing ``NL_X_NB_BS_HS`` one-plane transfer path.
    No bytes are reordered or interpreted as attention K/V.
    """
    ...
    def apply(
        ...
        """Convert ``[NB, 1, 1, context]`` to ``[NB, BS, opaque]``."""
        assert isinstance(kv_cache, torch.Tensor), (
            "single-layer KV cache must be a torch.Tensor"
        )
        return kv_cache.view(kv_cache.shape[0], spec.block_size, -1)
