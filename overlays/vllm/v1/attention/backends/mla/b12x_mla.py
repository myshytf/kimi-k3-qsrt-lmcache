...
# Adds the tested Kimi K3 TP8/DCP8 B12X MLA planning and metadata path.
...
def _max_dcp_local_cache_tokens(
    ...
    """Return the largest round-robin KV shard held by one DCP rank."""
    ...
    if dcp_size < 1 or interleave < 1:
        raise ValueError(
            "B12X_MLA requires positive DCP size and KV interleave, got "
...
class B12xMLAMetadata(MLACommonMetadata):
    ...
    dense_mla_output: torch.Tensor | None = None
...
class B12xMLAMetadataBuilder(MLACommonMetadataBuilder[B12xMLAMetadata]):
    ...
    def __init__(
        ...
        self._dense_mla_plan = _create_dense_mla_plan(
            ...
            num_q_heads=self._effective_heads,
        ...
        # These addresses are retained by full decode CUDA graphs. The layers
        # represented by one metadata builder execute serially, so one arena and
        # output buffer can safely be shared by all of them.
        self._dense_mla_scratch = torch.empty(
            scratch_shape, dtype=scratch_dtype, device=device
        )
        self._dense_mla_output = torch.empty(
            (max_dense_mla_rows, self._effective_heads, _K3_KV_LORA_RANK),
            dtype=torch.bfloat16,
            device=device,
        )
        ...
        logger.info_once(
            "B12X dense K3 MLA DCP plan: local_heads=%d, effective_heads=%d, "
            "DCP=%d, page_size=%d, max_decode_rows=%d, "
            "local_cache_tokens=%d, splits=%d",
            self.num_heads,
    ...
    def build(
        ...
        metadata.dense_mla_output = self._dense_mla_output
...
class B12xMLABackend(MLACommonBackend):
    ...
    def supports_combination(
        ...
        if effective_heads <= 0:
            return (
                "B12X_MLA requires a positive gathered query-head count, got "
                f"local={local_heads}, DCP={dcp_size}."
            )
...
class B12xMLAImpl(MLACommonImpl[B12xMLAMetadata]):
    ...
    def forward_mqa(
        ...
        if int(q.shape[1]) != self._effective_heads:
            raise ValueError(
                "B12X_MLA expected the DCP-gathered query-head count "
                f"{self._effective_heads}, got {q.shape[1]}."
            )

        output_storage = getattr(attn_metadata, "dense_mla_output", None)
        if output_storage is None:
            output = torch.empty(
                (total_q, self._effective_heads, self.kv_lora_rank),
                dtype=torch.bfloat16,
                device=q.device,
            )
        else:
            if (
                int(output_storage.shape[0]) < total_q
                or int(output_storage.shape[1]) != self._effective_heads
            ):
                raise ValueError(
                    "B12X_MLA capture-stable output capacity does not match the "
                    f"live query: storage={tuple(output_storage.shape)}, "
                    f"query={tuple(q.shape)}."
                )
            output = output_storage[:total_q]
